...
class _SearchPageState extends State<SearchPage> {
  ...
  initiateSearchMethod() async {
    if(searchController.text.isNotEmpty){
      ...
      await MessagingService().serachByName(searchController.text).then((snapshot){
         setState(() {
  ...
  joinedOrNot(String userName, String groupId, String groupName, String admin) async {
    bool userJoined = await MessagingService(uid: user!.uid).isUserJoined(groupName, groupId, userName);
    setState(() {
      isJoined = userJoined;
    });
  }
    ...
    subtitle: Text("Admin: ${getName(admin)}"),
      trailing: InkWell(
        onTap: ()async{
          ...
          await MessagingService(uid: user!.uid).toggleGroupJoin(groupId, userName, groupName);
          if(isJoined){
            setState(() {
              isJoined = !isJoined;
