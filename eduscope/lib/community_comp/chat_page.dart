...
import 'package:eduscope_2023/widget_tree.dart';
...
class _ChatPageState extends State<ChatPage> {
  ...
  getChatandAdmin() async {
    var chatResult = await MessagingService().getChats(widget.groupId);
    if (chatResult != null) {
      setState(() {
        chats = chatResult;
      });
    ...
    var adminResult = await MessagingService().getGroupAdmin(widget.groupId);
    if (adminResult != null) {
      setState(() {
        admin = adminResult;
      });
  ...
  sendMessage() {
    if (messaagesController.text.isNotEmpty) {
      ...
      MessagingService().sendMessage(widget.groupId, chatMessageMap);
      setState(() {
